<template>
  <div class="w-full max-w-[560px] mx-auto py-16">
    ...
    <p class="mt-1 text-sm leading-6 text-gray-600 mb-4">This information will be displayed publicly so be careful
                what you share.</p>    <form @submit.prevent="submitForm" ref="form" class="space-y-8">
      <div>
        <label
          for="username"
        ...
        >
          ...
          <input
            ...
            class="block flex-1 border-0 bg-transparent h-[36px] py-1.5 pl-1 text-gray-900 placeholder:text-gray-400 focus:ring-0 sm:text-sm sm:leading-6"
            placeholder="janesmith"
            @blur="showNextQuestion('websiteUrl')"
            required
          />
        </div>
      </div>
      ...
      <div class="col-span-full">
        <label
          ...
          <div class="mt-2 flex items-center gap-x-3">
            ...
            <div v-if="formData.photo">Selected File: {{ formData.photo }}</div>
          </div>
        </label>
      </div>
      ...
      <div class="col-span-full">
        ...
        >
          <div class="text-center">
            ...
            <div
              class="mt-4 flex text-sm justify-center leading-6 text-gray-600"
            >
              <label
                for="cover"
            ...
            <div v-if="formData.cover">Selected File: {{ formData.cover }}</div>
          </div>
        </div>
      </div>
...
const showWebsiteUrl = ref(false);
...
const handleCover = (event) => {
  ...
  formData.value.cover = file ? file.name : "";
};
...
const submitForm = () => {
  if (
    ...
    !formData.value.email || !formData.value.about || !formData.value.cover || !formData.value.photo
  ) {
